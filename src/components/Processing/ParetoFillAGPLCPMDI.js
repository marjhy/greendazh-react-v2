...
const ParetoFillAGPLBPM = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto GEA Downtime Internal / Month ' + monthName + ' ' + value
  );

  const handleChange = (event) => {
    setMOnth(event.target.value);
    if (event.target.value === 1) {
      setTitle('Pareto GEA Downtime Internal / Month January '+ value);
    }
    if (event.target.value === 2) {
      setTitle('Pareto GEA Downtime Internal / Month February '+ value);
    }
    if (event.target.value === 3) {
      setTitle('Pareto GEA Downtime Internal / Month March '+ value);
    }
    if (event.target.value === 4) {
      setTitle('Pareto GEA Downtime Internal / Month April '+ value);
    }
    if (event.target.value === 5) {
      setTitle('Pareto GEA Downtime Internal / Month May '+ value);
    }
    if (event.target.value === 6) {
      setTitle('Pareto GEA Downtime Internal / Month June '+ value);
    }
    if (event.target.value === 7) {
      setTitle('Pareto GEA Downtime Internal / Month July '+ value);
    }
    if (event.target.value === 8) {
      setTitle('Pareto GEA Downtime Internal / Month August '+ value);
    }
    if (event.target.value === 9) {
      setTitle('Pareto GEA Downtime Internal / Month September '+ value);
    }
    if (event.target.value === 10) {
      setTitle('Pareto GEA Downtime Internal / Month October '+ value);
    }
    if (event.target.value === 11) {
      setTitle('Pareto GEA Downtime Internal / Month November '+ value);
    }
    if (event.target.value === 12) {
      setTitle('Pareto GEA Downtime Internal / Month December '+ value);
    }
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get(`http://10.24.7.70:8080/OeeMilAGPLCPMDI/${value}`)
      .then((res) => {
        console.log(res);
          ...
          <TextField   
           ...
           id='demo-simple-select-outlined2'
            ...
            onChange={(e) => {
              ...
              setValue(value);
              // setTitle('Pareto GEA Downtime Internal / Month ' + month + ' ' + value);
              setTitle('Pareto GEA Downtime Internal / Month ?? ' + value);
            }}
            variant="outlined"
          /><br/>  
