...
const ParetoFilAGPLDPWDI = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto STR 125 Downtime Internal / Week #' + minggu
  );

  const handleChange2 = (event) => {
    setMinggu(event.target.value);
    setTitle('Pareto STR 125 Downtime Internal / Week #' + event.target.value);
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get('http://10.24.7.70:8080/OeeStrAGPLDPWDI')
      .then((res) => {
        console.log(res);
...
export default ParetoFilAGPLDPWDI;
