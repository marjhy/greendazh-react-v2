...
const ParetoFillAGPLBPM = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto STR 125 Downtime External / Month ' + monthName + ' ' + value
  );
 
  const handleChange = (event) => {
    setMOnth(event.target.value);
    if (event.target.value === 1) {
      setTitle('Pareto STR 125 Downtime External / Month January '+ value);
    }
    if (event.target.value === 2) {
      setTitle('Pareto STR 125 Downtime External / Month February '+ value);
    }
    if (event.target.value === 3) {
      setTitle('Pareto STR 125 Downtime External / Month March '+ value);
    }
    if (event.target.value === 4) {
      setTitle('Pareto STR 125 Downtime External / Month April '+ value);
    }
    if (event.target.value === 5) {
      setTitle('Pareto STR 125 Downtime External / Month May '+ value);
    }
    if (event.target.value === 6) {
      setTitle('Pareto STR 125 Downtime External / Month June '+ value);
    }
    if (event.target.value === 7) {
      setTitle('Pareto STR 125 Downtime External / Month July '+ value);
    }
    if (event.target.value === 8) {
      setTitle('Pareto STR 125 Downtime External / Month August '+ value);
    }
    if (event.target.value === 9) {
      setTitle('Pareto STR 125 Downtime External / Month September '+ value);
    }
    if (event.target.value === 10) {
      setTitle('Pareto STR 125 Downtime External / Month October '+ value);
    }
    if (event.target.value === 11) {
      setTitle('Pareto STR 125 Downtime External / Month November '+ value);
    }
    if (event.target.value === 12) {
      setTitle('Pareto STR 125 Downtime External / Month December '+ value);
    }
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get(`http://10.24.7.70:8080/OeeStrAGPLDPMDE/${value}`)
      .then((res) => {
        console.log(res);
          ...
          <TextField   
           ...
           id='demo-simple-select-outlined2'
            ...
            onChange={(e) => {
              ...
              setValue(value);
              // setTitle('Pareto STR 125 Downtime External / Month ' + month + ' ' + value);
              setTitle('Pareto STR 125 Downtime External / Month ?? ' + value);
            }}
            variant="outlined"
          /><br/>  
