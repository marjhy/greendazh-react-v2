...
const ParetoFilAGPLBPWDE = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto STR 500/1000 Downtime Internal / Week #' + minggu
  );

  const handleChange2 = (event) => {
    setMinggu(event.target.value);
    setTitle(
      'Pareto STR 500/1000 Downtime Internal / Week #' + event.target.value
    );
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get('http://10.24.7.70:8080/OeeStrAGPLBPWDE')
      .then((res) => {
        console.log(res);
...
export default ParetoFilAGPLBPWDE;
