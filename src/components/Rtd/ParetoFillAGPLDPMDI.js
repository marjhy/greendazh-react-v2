...
const ParetoFillAGPLDPMDI = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto RTD 125 Downtime Internal / Month ' + monthName
  );

  const handleChange = (event) => {
    setMOnth(event.target.value);
    if (event.target.value === 1) {
      setTitle('Pareto RTD 125 Downtime Internal / Month January');
    }
    if (event.target.value === 2) {
      setTitle('Pareto RTD 125 Downtime Internal / Month February');
    }
    if (event.target.value === 3) {
      setTitle('Pareto RTD 125 Downtime Internal / Month March');
    }
    if (event.target.value === 4) {
      setTitle('Pareto RTD 125 Downtime Internal / Month April');
    }
    if (event.target.value === 5) {
      setTitle('Pareto RTD 125 Downtime Internal / Month May');
    }
    if (event.target.value === 6) {
      setTitle('Pareto RTD 125 Downtime Internal / Month June');
    }
    if (event.target.value === 7) {
      setTitle('Pareto RTD 125 Downtime Internal / Month July');
    }
    if (event.target.value === 8) {
      setTitle('Pareto RTD 125 Downtime Internal / Month August');
    }
    if (event.target.value === 9) {
      setTitle('Pareto RTD 125 Downtime Internal / Month September');
    }
    if (event.target.value === 10) {
      setTitle('Pareto RTD 125 Downtime Internal / Month October');
    }
    if (event.target.value === 11) {
      setTitle('Pareto RTD 125 Downtime Internal / Month November');
    }
    if (event.target.value === 12) {
      setTitle('Pareto RTD 125 Downtime Internal / Month December');
    }
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get('http://10.24.7.70:8080/OeeRtdAGPLDPMDI')
      .then((res) => {
        console.log(res);
...
export default ParetoFillAGPLDPMDI;
