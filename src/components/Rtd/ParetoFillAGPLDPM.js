...
const ParetoFillAGPLDPM = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto RTD 125 Downtime All / Month ' + monthName
  );

  const handleChange = (event) => {
    setMOnth(event.target.value);
    if (event.target.value === 1) {
      setTitle('Pareto RTD 125 Downtime All / Month January');
    }
    if (event.target.value === 2) {
      setTitle('Pareto RTD 125 Downtime All / Month February');
    }
    if (event.target.value === 3) {
      setTitle('Pareto RTD 125 Downtime All / Month March');
    }
    if (event.target.value === 4) {
      setTitle('Pareto RTD 125 Downtime All / Month April');
    }
    if (event.target.value === 5) {
      setTitle('Pareto RTD 125 Downtime All / Month May');
    }
    if (event.target.value === 6) {
      setTitle('Pareto RTD 125 Downtime All / Month June');
    }
    if (event.target.value === 7) {
      setTitle('Pareto RTD 125 Downtime All / Month July');
    }
    if (event.target.value === 8) {
      setTitle('Pareto RTD 125 Downtime All / Month August');
    }
    if (event.target.value === 9) {
      setTitle('Pareto RTD 125 Downtime All / Month September');
    }
    if (event.target.value === 10) {
      setTitle('Pareto RTD 125 Downtime All / Month October');
    }
    if (event.target.value === 11) {
      setTitle('Pareto RTD 125 Downtime All / Month November');
    }
    if (event.target.value === 12) {
      setTitle('Pareto RTD 125 Downtime All / Month December');
    }
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get('http://10.24.7.70:8080/OeeRtdAGPLDPM')
      .then((res) => {
        console.log(res);
