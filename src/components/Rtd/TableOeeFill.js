...
const TableOeeFill = () => {
  ...
  const downloadExcel = () => {
    const newData = dataOeeWB.map((row) => {
      delete row.tableData;
      return row;
    });
    const workSheet = XLSX.utils.json_to_sheet(newData);
    const workBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workBook, workSheet, 'GrendazhData');
    //Buffer
    let buf = XLSX.write(workBook, { bookType: 'xlsx', type: 'buffer' });
    //Binary string
    XLSX.write(workBook, { bookType: 'xlsx', type: 'binary' });
    //Download
    XLSX.writeFile(workBook, 'GreendazhData.xlsx');
  };
  ...
  return (
    <Container className='container'>
      ...
      <Paper className={classes.root}>
        ...
        {/* 
        <IconButton aria-label='delete' onClick={downloadExcel}>
          <DownloadIcon />
        </IconButton> */}
