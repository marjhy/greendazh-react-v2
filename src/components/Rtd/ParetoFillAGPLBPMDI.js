...
const ParetoFillAGPLBPM = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto RTD 250/150 Downtime Internal / Month ' + monthName + ' ' + value
  );
 
  const handleChange = (event) => {
    setMOnth(event.target.value);
    if (event.target.value === 1) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month January '+ value);
    }
    if (event.target.value === 2) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month February '+ value);
    }
    if (event.target.value === 3) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month March '+ value);
    }
    if (event.target.value === 4) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month April '+ value);
    }
    if (event.target.value === 5) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month May '+ value);
    }
    if (event.target.value === 6) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month June '+ value);
    }
    if (event.target.value === 7) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month July '+ value);
    }
    if (event.target.value === 8) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month August '+ value);
    }
    if (event.target.value === 9) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month September '+ value);
    }
    if (event.target.value === 10) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month October '+ value);
    }
    if (event.target.value === 11) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month November '+ value);
    }
    if (event.target.value === 12) {
      setTitle('Pareto RTD 250/150 Downtime Internal / Month December '+ value);
    }
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get(`http://10.24.7.70:8080/OeeRtdAGPLBPMDI/${value}`)
      .then((res) => {
        console.log(res);
          ...
          <TextField   
           ...
           id='demo-simple-select-outlined2'
            ...
            onChange={(e) => {
              ...
              setValue(value);
              // setTitle('Pareto RTD 250/150 Downtime Internal / Month ' + month + ' ' + value);
              setTitle('Pareto RTD 250/150 Downtime Internal / Month ?? ' + value);
            }}
            variant="outlined"
          /><br/>  
