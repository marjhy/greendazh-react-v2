...
const ParetoFilAGPLDPWDE = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto RTD 125 Downtime External / Week #' + minggu
  );

  const handleChange2 = (event) => {
    setMinggu(event.target.value);
    setTitle('Pareto RTD 125 Downtime External / Week #' + event.target.value);
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get('http://10.24.7.70:8080/OeeRtdAGPLDPWDE')
      .then((res) => {
        console.log(res);
...
export default ParetoFilAGPLDPWDE;
