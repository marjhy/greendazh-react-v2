...
const ParetoFillAGPLBPM = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto Line B Downtime All / Month ' + monthName + ' ' + value
  );
  ...
  const handleChange = (event) => {
    ...
    if (event.target.value === 2) {
      setTitle('Pareto Line B Downtime All / Month February '+ value);
    }
    if (event.target.value === 3) {
      setTitle('Pareto Line B Downtime All / Month March '+ value);
    }
    if (event.target.value === 4) {
      setTitle('Pareto Line B Downtime All / Month April '+ value);
    }
    if (event.target.value === 5) {
      setTitle('Pareto Line B Downtime All / Month May '+ value);
    }
    if (event.target.value === 6) {
      setTitle('Pareto Line B Downtime All / Month June '+ value);
    }
    if (event.target.value === 7) {
      setTitle('Pareto Line B Downtime All / Month July '+ value);
    }
    if (event.target.value === 8) {
      setTitle('Pareto Line B Downtime All / Month August '+ value);
    }
    if (event.target.value === 9) {
      setTitle('Pareto Line B Downtime All / Month September '+ value);
    }
    if (event.target.value === 10) {
      setTitle('Pareto Line B Downtime All / Month October '+ value);
    }
    if (event.target.value === 11) {
      setTitle('Pareto Line B Downtime All / Month November '+ value);
    }
    if (event.target.value === 12) {
      setTitle('Pareto Line B Downtime All / Month December '+ value);
    }
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get(`http://10.24.7.70:8080/OeeFilAGPLBPM/${value}`)
      .then((res) => {
        console.log(res);
