...
const ParetoFillAGPLBPM = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto Ricotta 250 Downtime All / Month ' + monthName + ' ' + value
  );
 
  const handleChange = (event) => {
    setMOnth(event.target.value);
    if (event.target.value === 1) {
      setTitle('Pareto Ricotta 250 Downtime All / Month January '+ value);
    }
    if (event.target.value === 2) {
      setTitle('Pareto Ricotta 250 Downtime All / Month February '+ value);
    }
    if (event.target.value === 3) {
      setTitle('Pareto Ricotta 250 Downtime All / Month March '+ value);
    }
    if (event.target.value === 4) {
      setTitle('Pareto Ricotta 250 Downtime All / Month April '+ value);
    }
    if (event.target.value === 5) {
      setTitle('Pareto Ricotta 250 Downtime All / Month May '+ value);
    }
    if (event.target.value === 6) {
      setTitle('Pareto Ricotta 250 Downtime All / Month June '+ value);
    }
    if (event.target.value === 7) {
      setTitle('Pareto Ricotta 250 Downtime All / Month July '+ value);
    }
    if (event.target.value === 8) {
      setTitle('Pareto Ricotta 250 Downtime All / Month August '+ value);
    }
    if (event.target.value === 9) {
      setTitle('Pareto Ricotta 250 Downtime All / Month September '+ value);
    }
    if (event.target.value === 10) {
      setTitle('Pareto Ricotta 250 Downtime All / Month October '+ value);
    }
    if (event.target.value === 11) {
      setTitle('Pareto Ricotta 250 Downtime All / Month November '+ value);
    }
    if (event.target.value === 12) {
      setTitle('Pareto Ricotta 250 Downtime All / Month December '+ value);
    }
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get(`http://10.24.7.70:8080/OeeChsAGPLCPM/${value}`)
      .then((res) => {
        console.log(res);
          ...
          <TextField   
           ...
           id='demo-simple-select-outlined2'
            ...
            onChange={(e) => {
              ...
              setValue(value);
              // setTitle('Pareto Ricotta 250 Downtime All / Month ' + month + ' ' + value);
              setTitle('Pareto Ricotta 250 Downtime All / Month ?? ' + value);
            }}
            variant="outlined"
          /><br/>  
