...
const ParetoFilAGPLCPWDE = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto Ricotta 250 Downtime External / Week #' + minggu
  );

  const handleChange2 = (event) => {
    setMinggu(event.target.value);
    setTitle(
      'Pareto Ricotta 250 Downtime External / Week #' + event.target.value
    );
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get('http://10.24.7.70:8080/OeeChsAGPLCPWDE')
      .then((res) => {
        console.log(res);
...
export default ParetoFilAGPLCPWDE;
