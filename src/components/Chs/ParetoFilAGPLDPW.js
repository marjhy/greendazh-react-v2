...
const ParetoFilAGPLDPW = () => {
  ...
  const [title, setTitle] = useState(
    'Pareto Mozza 200 Downtime All / Week #' + minggu
  );

  const handleChange2 = (event) => {
    setMinggu(event.target.value);
    setTitle('Pareto Mozza 200 Downtime All / Week #' + event.target.value);
  };
  ...
  const getDataFromAPI = () => {
    ...
    axios
      .get('http://10.24.7.70:8080/OeeChsAGPLDPW')
      .then((res) => {
        console.log(res);
...
export default ParetoFilAGPLDPW;
