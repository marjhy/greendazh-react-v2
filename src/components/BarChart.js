...
const BarChart = () => {
  ...
  const chartPie1 = () => {
    ...
    let fs = [];
    axios
      .get('http://10.24.7.70:8080/findstC')
      .then((res) => {
        //console.log(res);
        for (const dataObj of res.data) {
          qf.push(parseInt(dataObj.Qty));
          fs.push(dataObj.finding_audit_status);
        }
        setChartDataFst({
          labels: fs,
          datasets: [
    ...
    axios
      ...
      .then((res) => {
        ...
        for (const dataObj2 of res.data) {
          ...
          qtyChs.push(parseInt([dataObj2['CHYOG PRODUCTION']].join(',')));
        ...
        <Container id='left'>
          <Pie
            ...
            options={{
              responsive: true,
              title: {
                text: 'CORRECTIVE STATUS OPEN CLOSE %',
                display: true,
                fontSize: 18,
                fontFamily: '"Segoe UI Light"',
              },
